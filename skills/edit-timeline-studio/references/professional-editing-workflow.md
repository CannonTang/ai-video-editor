...
Keep one primary emphasis per shot. Prefer an eased keyframed crop with a still reading hold over a persistent rectangle when the target is static and high-resolution. Use a box only for precise spatial disambiguation, and never show multiple competing boxes unless the narration explicitly compares those regions. For narrated edits, first synthesize and accept separate semantic audio segments, normally space them `0.5–0.8s` apart, and lock the resulting audio spine. Time every acquire, hold, release, and picture cut from that final audio rather than stretching narration to match a prebuilt visual template.
